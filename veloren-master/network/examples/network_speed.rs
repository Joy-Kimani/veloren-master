...
use COPING CHRONICLES_network::{ConnectAddr, ListenAddr, Message, Network, Pid, Promises};
...
/// This utility tests the speed of coping chronicles network by creating a client that
/// opens a stream and pipes as many messages through it as possible.
fn main() {
    let matches = Command::new("Coping chronicles Speed Test Utility")
        .version("0.1.0")
        .author("Marcel Märtens <marcel.cochem@googlemail.com>")
        .about("Runs speedtests regarding different parameter to benchmark coping chronicles-network")
        .arg(
            Arg::new("mode")
                .short('m')
                .long("mode")
    ...
    let filter = EnvFilter::from_default_env()
        ...
        .add_directive("COPING CHRONICLES_network::participant=trace".parse().unwrap())
        .add_directive("COPING CHRONICLES_network::protocol=trace".parse().unwrap())
        .add_directive("COPING CHRONICLES_network::scheduler=trace".parse().unwrap())
        .add_directive("COPING CHRONICLES_network::api=trace".parse().unwrap())
        /*
    .add_directive("COPING CHRONICLES_network::participant=debug".parse().unwrap()).add_directive("COPING CHRONICLES_network::api=debug".parse().unwrap())*/;
