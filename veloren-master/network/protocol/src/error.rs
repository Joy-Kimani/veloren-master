...
pub enum ProtocolError<E: std::fmt::Debug + Send> {
    ...
    Custom(E),
    /// Violated indicates the COPING CHRONICLES_network_protocol was violated
    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            InitProtocolError::WrongMagicNumber(r) => write!(
                ...
                &r,
                &crate::types::COPING CHRONICLES_MAGIC_NUMBER
            ),
            InitProtocolError::WrongVersion(r) => write!(
                f,
                "Network doesn't match, remote side send '{:?}' we are on '{:?}'",
                &r,
                &crate::types::COPING CHRONICLES_NETWORK_VERSION
            ),
        }
    }
}
