use crate::{
    ...
    frame::InitFrame,
    types::{
        Pid, STREAM_ID_OFFSET1, STREAM_ID_OFFSET2, Sid, COPING CHRONICLES_MAGIC_NUMBER,
        COPING CHRONICLES_NETWORK_VERSION,
    },
};
    ...
    E: std::fmt::Debug + Send,
{
    ...
    async fn initialize(
        ...
        const WRONG_NUMBER: &str = "Handshake does not contain the magic number required by \
                                    coping chronicles server.\nWe are not sure if you are a valid coping chronicles \
        ...
        if initializer {
            drain
                .send(InitFrame::Handshake {
                    magic_number: COPING CHRONICLES_MAGIC_NUMBER,
                    version: COPING CHRONICLES_NETWORK_VERSION,
                })
                .await?;
        }
        ...
        match sink.recv().await? {
            InitFrame::Handshake {
                ...
                if magic_number != COPING CHRONICLES_MAGIC_NUMBER {
                ...
                } else if version[0] != COPING CHRONICLES_NETWORK_VERSION[0]
                    || version[1] != COPING CHRONICLES_NETWORK_VERSION[1]
                {
                    ...
                    drain
                        .send(InitFrame::Raw(
                            format!(
                                "{} Our Version: {:?}\nYour Version: {:?}\nClosing the connection",
                                WRONG_VERSION, COPING CHRONICLES_NETWORK_VERSION, version,
                            )
                            .as_bytes()
                            .to_vec(),
                        ))
