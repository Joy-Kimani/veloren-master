    ...
    pub fn new(local_pid: &Pid) -> Result<Self, Box<dyn Error>> {
        ...
        let opts = Opts::new("network_info", "Static Network information")
            .const_label(
                ...
                format!(
                    "{}.{}.{}",
                    &network_protocol::COPING CHRONICLES_NETWORK_VERSION[0],
                    &network_protocol::COPING CHRONICLES_NETWORK_VERSION[1],
                    &network_protocol::COPING CHRONICLES_NETWORK_VERSION[2]
                ),
            )
