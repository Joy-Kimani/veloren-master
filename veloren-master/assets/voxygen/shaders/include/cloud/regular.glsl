...
vec4 cloud_at(vec3 pos, float dist, vec3 dir, out vec3 emission, out float not_underground) {
    ...
    // it can't be since Coping chronicles's world is flat, not spherical.
    float atmosphere_alt = CLOUD_AVG_ALT + 40000.0;
    // Coping chronicles's world is flat. This is, to put it mildly, somewhat non-physical. With the earth as an infinitely-big
