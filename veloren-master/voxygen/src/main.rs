...
use COPING CHRONICLES_voxygen::singleplayer::SingleplayerState;
use COPING CHRONICLES_voxygen::{
    GlobalState,
    audio::AudioFrontend,
    cli, panic_handler,
    profile::Profile,
...
#[cfg(feature = "egui-ui")]
use COPING CHRONICLES_voxygen::ui::egui::EguiState;
...
fn main() {
    ...
    // Determine Voxygen's config directory either by env var or placed in coping chronicles's
    // userdata folder
    ...
    // Create window
    use COPING CHRONICLES_voxygen::{error::Error, render::RenderError};
    ...
    let discord = if settings.networking.enable_discord_integration {
        COPING CHRONICLES_voxygen::discord::Discord::start(&tokio_runtime)
    } else {
        COPING CHRONICLES_voxygen::discord::Discord::Inactive
    };
