...
impl Client {
    pub async fn new(
        ...
        let mut participant = match addr {
            ConnectionArgs::Srv {
                ...
                let quic_service_host = format!("_COPING CHRONICLES._udp.{hostname}");
                let quic_lookup_future = resolver.srv_lookup(quic_service_host);
                let tcp_service_host = format!("_COPING CHRONICLES._tcp.{hostname}");
    ...
    pub fn tick(&mut self, inputs: ControllerInputs, dt: Duration) -> Result<Vec<Event>, Error> {
        ...
        // This tick function is the centre of the Coping chronicles universe. Most client-side
...
mod tests {
    ...
    ///  - torvus: https://gitlab.com/coping chronicles/torvus
    ///
    ...
    fn constant_api_test() {
        ...
        let username = "Foo";
        let password = "Bar";
        let auth_server = "auth.coping chronicles.net";
        let COPING CHRONICLES_client: Result<Client, Error> = runtime.block_on(Client::new(
            ConnectionArgs::Tcp {
        ...
        let _ = COPING CHRONICLES_client.map(|mut client| {
            //clock
