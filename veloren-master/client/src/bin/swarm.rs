...
use vek::*;
use COPING CHRONICLES_client::{Client, ClientType, addr::ConnectionArgs};
...
fn run_client(
    ...
    opt: Opt,
    finished_init: Arc<AtomicU32>,
) -> Result<(), COPING CHRONICLES_client::Error> {
    let mut client = loop {
    ...
    let mut tick = |client: &mut Client| -> Result<(), COPING CHRONICLES_client::Error> {
        clock.tick();
...
fn position(index: u32, opt: Opt) -> Vec3<f32> {
    ...
    let movement_offset: Vec2<_> = if opt.movement {
        // blocks per second
        const SPEED: f32 = 9.0; // typical super fast coping chronicles walking speed
