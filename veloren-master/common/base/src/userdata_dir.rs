...
const COPING CHRONICLES_USERDATA_ENV: &str = "COPING CHRONICLES_USERDATA";
...
/// Determines common user data directory used by coping chronicles frontends.
///
/// The first specified in this list is used.
///   1. The COPING CHRONICLES_USERDATA environment variable
///   2. The COPING CHRONICLES_USERDATA_STRATEGY environment variable
///   3. The CARGO_MANIFEST_DIR/userdata or CARGO_MANIFEST_DIR/../userdata
///      depending on if a workspace if being used
///
/// ### `COPING CHRONICLES_USERDATA_STRATEGY` environment variable
///
...
pub fn userdata_dir(workspace: bool, strategy: Option<&str>, manifest_dir: &str) -> PathBuf {
    // 1. The COPING CHRONICLES_USERDATA environment variable
    std::env::var_os(COPING CHRONICLES_USERDATA_ENV)
        .map(PathBuf::from)
        // 2. The COPING CHRONICLES_USERDATA_STRATEGY environment variable
        .or_else(|| match strategy {
            // "system" => system specific project data directory
            Some(s) if s.eq_ignore_ascii_case("system") => Some(directories_next::ProjectDirs::from("net", "coping chronicles", "coping chronicles")
            ...
            Some(s) => {
                warn!(
                    "Compiled with an invalid COPING CHRONICLES_USERDATA_STRATEGY: \"{}\". \
        ...
        .unwrap_or_else(|| {
            ...
            if path.exists() && exe_path.starts_with(&path) {
                ...
                warn!(
                    ...
                    and was not compiled with COPING CHRONICLES_USERDATA_STRATEGY set to \"system\" or \"executable\". \
                    ...
                    path.display(),
                    COPING CHRONICLES_USERDATA_ENV,
                );
                path
            }
        })
...
macro_rules! userdata_dir_workspace {
    () => {
        $crate::userdata_dir::userdata_dir(
            true,
            option_env!("COPING CHRONICLES_USERDATA_STRATEGY"),
