...
pub const COPING CHRONICLES_VERSION_STAGE: &str = "Pre-Alpha";

lazy_static::lazy_static! {
    pub static ref GIT_VERSION: String =
        std::env::var("COPING CHRONICLES_GIT_VERSION").unwrap_or_else(|_| GIT_VERSION_BUILD.to_string());
    pub static ref GIT_TAG: String =
        std::env::var("COPING CHRONICLES_GIT_TAG").unwrap_or_else(|_| GIT_TAG_BUILD.to_string());
    ...
    pub static ref DISPLAY_VERSION: String = if GIT_TAG.is_empty() {
        format!("{}-{}", COPING CHRONICLES_VERSION_STAGE, *GIT_DATE)
    } else {
        format!("{}-{}", COPING CHRONICLES_VERSION_STAGE, GIT_TAG.as_str())
    };
