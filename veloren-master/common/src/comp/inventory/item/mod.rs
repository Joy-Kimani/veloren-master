...
pub struct ItemDef {
    ...
    /// prepended with `coping chronicles.core` is reserved for coping chronicles functions.
...
impl assets::Compound for ItemDef {
    fn load(cache: assets::AnyCache, specifier: &assets::SharedString) -> Result<Self, BoxedError> {
        if specifier.starts_with("coping chronicles.core.") {
            return Err(format!(
                "Attempted to load an asset from a specifier reserved for core coping chronicles functions. \
                 Specifier: {}",
                specifier
            )
            .into());
...
mod tests {
    ...
    #[test]
    // All items in Coping chronicles should have localization.
