...
pub(crate) const COPING CHRONICLES_HEADER: [u8; 7] = [b'v', b'e', b'l', b'o', b'r', b'e', b'n'];
...
pub(crate) const MAX_REQUEST_SIZE: usize = MAX_REQUEST_CONTENT_SIZE + COPING CHRONICLES_HEADER.len() + 2;
...
impl RawQueryServerRequest {
    ...
    pub fn serialize(&self) -> Result<Vec<u8>, protocol::Error> {
        ...
        const _: () = assert!(MAX_RESPONSE_SIZE + COPING CHRONICLES_HEADER.len() <= MAX_REQUEST_SIZE);
        buf.resize(MAX_RESPONSE_SIZE.max(buf.len()), 0);
        buf.extend(COPING CHRONICLES_HEADER);
        Ok(buf)
    }
}
