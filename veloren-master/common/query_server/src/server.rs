...
use crate::{
    proto::{
        ...
        RawQueryServerRequest, RawQueryServerResponse, ServerInfo, COPING CHRONICLES_HEADER, VERSION,
    },
...
impl QueryServer {
    ...
    /// using `coping chronicles-common-frontend`.
    pub async fn run(&mut self, metrics: Arc<Mutex<Metrics>>) -> Result<(), tokio::io::Error> {
        ...
        loop {
            ...
            let msg_buf = if Self::validate_datagram(raw_msg_buf) {
                ...
                &raw_msg_buf[2..(raw_msg_buf.len() - COPING CHRONICLES_HEADER.len())]
            } else {
    ...
    fn validate_datagram(data: &[u8]) -> bool {
        let len = data.len();
        // Require 2 extra bytes for version (currently unused)
        if len < MAX_RESPONSE_SIZE.max(COPING CHRONICLES_HEADER.len() + 2) {
            trace!(?len, "Datagram too short");
            false
        } else if len > MAX_REQUEST_SIZE {
            trace!(?len, "Datagram too large");
            false
        } else if data[(len - COPING CHRONICLES_HEADER.len())..] != COPING CHRONICLES_HEADER {
