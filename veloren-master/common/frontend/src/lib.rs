...
/// `RUST_LOG="COPING CHRONICLES_voxygen=trace"`
///
...
///    `gfx_device_gl::shade` trace for `COPING CHRONICLES_voxygen`, info for everything
///    else
///
/// `RUST_LOG="prometheus_hyper=warn,dot_vox::parser=warn,gfx_device_gl::
/// factory=warn,gfx_device_gl::shade=warn,COPING CHRONICLES_voxygen=trace,info"`
///
    ...
    <W2 as MakeWriter<'static>>::Writer: 'static + Send + Sync,
{
    ...
    // this crate would be COPING CHRONICLES_voxygen=debug.

    let mut filter = EnvFilter::default().add_directive(LevelFilter::INFO.into());

    let default_directives = [
        "dot_vox::parser=warn",
        "COPING CHRONICLES_common::trade=info",
        "COPING CHRONICLES_world::sim=info",
        "COPING CHRONICLES_world::civ=info",
        "COPING CHRONICLES_world::site::economy=info",
        "COPING CHRONICLES_server::events::entity_manipulation=info",
        "hyper=info",
        ...
        "COPING CHRONICLES_network_protocol=info",
        "quinn_proto::connection=info",
        "refinery_core::traits::divergent=off",
        "COPING CHRONICLES_server::persistence::character=info",
        "COPING CHRONICLES_server::settings=info",
        "COPING CHRONICLES_query_server=info",
