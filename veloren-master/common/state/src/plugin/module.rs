...
pub(crate) mod types_mod {
    wasmtime::component::bindgen!({
        path: "../../plugin/wit/coping chronicles.wit",
        world: "common-types",
    });
}
...
wasmtime::component::bindgen!({
    ...
    world: "plugin",
    with: {
        "coping chronicles:plugin/types@0.0.1": types_mod::coping chronicles::plugin::types,
        "coping chronicles:plugin/information@0.0.1/entity": Entity,
    },
});
...
mod animation_plugin {
    wasmtime::component::bindgen!({
        ...
        with: {
            "coping chronicles:plugin/types@0.0.1": super::types_mod::coping chronicles::plugin::types,
        },
    });
}
...
mod server_plugin {
    wasmtime::component::bindgen!({
        ...
        with: {
            "coping chronicles:plugin/types@0.0.1": super::types_mod::coping chronicles::plugin::types,
            "coping chronicles:plugin/information@0.0.1/entity": super::Entity,
        },
    });
}
...
use exports::coping chronicles::plugin::animation;
pub use types_mod::coping chronicles::plugin::types::{
    self, CharacterState, Dependency, Skeleton, Transform,
};
use coping chronicles::plugin::{actions, information};
...
impl PluginWrapper {
        ...
        <S as wasmtime::AsContext>::Data: std::marker::Send,
    {
        ...
        match self {
            PluginWrapper::Full(pl) => pl.COPING CHRONICLES_plugin_events().call_load(store, mode),
            PluginWrapper::Animation(pl) => pl.COPING CHRONICLES_plugin_events().call_load(store, mode),
            PluginWrapper::Server(pl) => pl.COPING CHRONICLES_plugin_events().call_load(store, mode),
        }
    }
        ...
        <S as wasmtime::AsContext>::Data: std::marker::Send,
    {
        match self {
            PluginWrapper::Full(pl) => pl
                .COPING CHRONICLES_plugin_server_events()
    ...
    fn create_body(&self, store: &mut StoreType, bodytype: i32) -> Option<animation::Body> {
        match self {
            PluginWrapper::Full(pl) => {
                let body_iface = pl.COPING CHRONICLES_plugin_animation().body();
...
impl PluginModule {
    ...
    pub fn new(name: String, wasm_data: &[u8]) -> Result<Self, PluginModuleError> {
        ...
        // register WASI and Coping chronicles methods with the runtime
    ...
    // Implementation of the commands called from coping chronicles and provided in plugins
    pub fn load_event(
        &mut self,
        ecs: &EcsWorld,
        mode: common::resources::GameMode,
