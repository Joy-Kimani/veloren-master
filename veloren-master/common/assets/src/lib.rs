...
pub trait AssetExt: Sized + Send + Sync + 'static {
    ...
    /// Example usage:
    /// ```no_run
    /// use COPING CHRONICLES_common_assets::{AssetExt, Image};
    ///
...
lazy_static! {
    ...
    /// 2. Install with package manager and run (assets probably in `/usr/share/coping chronicles/assets` while binary in `/usr/bin/`)
    ...
    pub static ref ASSETS_PATH: PathBuf = {
        ...
        // 1. COPING CHRONICLES_ASSETS environment variable
        if let Ok(var) = std::env::var("COPING CHRONICLES_ASSETS") {
            paths.push(var.into());
        }
        ...
        #[cfg(all(unix, not(target_os = "macos"), not(target_os = "ios"), not(target_os = "android")))]
        {
            if let Ok(result) = std::env::var("XDG_DATA_HOME") {
                paths.push(format!("{}/coping chronicles/", result).into());
            } else if let Ok(result) = std::env::var("HOME") {
                paths.push(format!("{}/.local/share/coping chronicles/", result).into());
            }

            if let Ok(result) = std::env::var("XDG_DATA_DIRS") {
                result.split(':').for_each(|x| paths.push(format!("{}/coping chronicles/", x).into()));
            } else {
                // Fallback
                let fallback_paths = vec!["/usr/local/share", "/usr/share"];
                for fallback_path in fallback_paths {
                    paths.push(format!("{}/coping chronicles/", fallback_path).into());
                }
            }
        }
...
pub mod asset_tweak {
    ...
    /// How not to use.
    /// ```should_panic
    /// use COPING CHRONICLES_common_assets::asset_tweak::{Specifier, tweak_expect};
    ///
    ...
    /// ```
    /// use std::fs;
    /// use COPING CHRONICLES_common_assets::{
    ///     ASSETS_PATH,
    ...
    /// use COPING CHRONICLES_common_assets::{ASSETS_PATH, tweak};
    ///
    /// // you need to create file first
    ...
    /// use COPING CHRONICLES_common_assets::{ASSETS_PATH, tweak_from};
    ///
