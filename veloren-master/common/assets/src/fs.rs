...
/// Loads assets from the default path or `COPING CHRONICLES_ASSETS_OVERRIDE` env if it is
/// set.
#[derive(Debug, Clone)]
...
impl FileSystem {
    pub fn new() -> io::Result<Self> {
        ...
        let override_dir = std::env::var_os("COPING CHRONICLES_ASSETS_OVERRIDE").and_then(|path| {
            RawFs::new(path)
        ...
        if !canary.starts_with("COPING CHRONICLES_CANARY_MAGIC") {
            panic!("Canary asset `canary.canary` was present but did not contain the expected data. This *heavily* implies that you've not correctly set up Git LFS (Large File Storage). Visit `https://book.coping chronicles.net/contributors/development-tools.html#git-lfs` for more information about setting up Git LFS.");
        }
